...
import student.IOHelper; // Assuming this helper class exists and is used as per spec

/**
 * Reads and parses the influencer data CSV file. Stores data for each
 * influencer by month using arrays within the Influencer class.
 * Handles invalid months and non-numeric data gracefully.
 */
public class InputFileReader {
     ...
     * Reads the specified file, parses the data for each row,
     * validates months, converts strings to integers using the helper method,
     * finds or creates Influencer objects, and adds the monthly data to those objects
     * within the InfluencerList.
     *
    ...
    public InputFileReader(String filename) throws IOException {
        ...
        while (inStream.hasNextLine()) {
            String line = inStream.nextLine().replaceAll(" ", ""); // Remove spaces (as per spec example)
            String[] values = line.split(",", -1); // Split, keeping empty fields
            ...
            if (influencer == null) {
                ...
                influencerList.add(influencer); // Add new influencer to the list
            }

            // Determine the month index and add the monthly data to the influencer object
            int monthIndex = getMonthIndex(month);
            if (monthIndex != -1) { // Should always be true due to isValidMonth check
     ...
     *
     * @param month The month string to check.
    ...
    /**
     * Gets the integer index for a given month string (0 for January, 1 for February, etc.).
     *
    ...
    private int getMonthIndex(String month) {
        switch (month) {
            ...
            default: return -1; // Should not happen if isValidMonth is called first
        }
    }

    /**
     * Helper method to convert a string to an integer.
     * Handles potential NumberFormatException by returning 0.
     *
